...
- **무료 티어**: 레벨 1-6 (초급~중급, Basics~Upper-Intermediate), AI 대화 3회/일, 광고 포함
- **프리미엄 전용**: 레벨 7-8 (고급, Advanced~K-Culture Master) + AI 대화 무제한 등
- **프리미엄**: 월 $7.99 / 연 $49.99 (연간 약 48% 할인)
- **무료 체험**: 연간 구독 시 3일 무료
- 프리미엄 기능 접근 시 → ProUpgrade 화면으로 이동 필수 (`FREE_MAX_LEVEL` 상수는 `src/data/lessons.ts`에서 관리)
- 결제 완료 전까지 프리미엄 콘텐츠 절대 노출 금지
- 출시 초기엔 무료 앱으로 배포, 사용자 확대 후 유료 전환 — 현재 버전은 레벨 7부터 즉시 결제 필요
